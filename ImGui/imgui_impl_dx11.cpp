...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
  ...
  {
    HRSRC rc = FindResource(g_mainHandle->GetDllHandle(), MAKEINTRESOURCE(IDR_RCDATA2), RT_RCDATA);
    if (!rc) return false;
  ...
  {
    HRSRC rc = FindResource(g_mainHandle->GetDllHandle(), MAKEINTRESOURCE(IDR_RCDATA1), RT_RCDATA);
    if (!rc) return false;
    HGLOBAL hglobal = LoadResource(g_mainHandle->GetDllHandle(), rc);
    if (!hglobal) return false;
    void* pCompiledShader = LockResource(hglobal);
...
ID3D11DeviceContext* ImGui_ImplDX11_GetContext()
{
